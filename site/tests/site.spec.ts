...
test("OS desktop hero는 메시지와 지도를 균형 잡힌 두 열로 보여 준다", async ({ page }) => {
  ...
  for (const viewport of [
    ...
    expect(Math.abs(geometry.visual.left - (geometry.copy.left + geometry.copy.width))).toBeLessThanOrEqual(1);
    expect(geometry.visual.width).toBeGreaterThan(geometry.hero.width * 0.5);
    expect(geometry.visual.width).toBeLessThan(geometry.hero.width * 0.65);
    // hero shell의 좌우 1px border를 제외한 실제 두 column 합계를 확인한다.
    expect(Math.abs(geometry.copy.width + geometry.visual.width - geometry.hero.width)).toBeLessThanOrEqual(3);
    expect(Math.abs(geometry.visual.top - geometry.hero.top)).toBeLessThanOrEqual(1);
    expect(Math.abs(geometry.visual.height - geometry.hero.height)).toBeLessThanOrEqual(1);
